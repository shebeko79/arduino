...
eat_bool eat_module_test_sim(u8 param1, u8 param2)
{
    ...
    }else if(2 == param1)
    {
        ...
        if(1 == param2)
        {
            eat_spi_init(EAT_SPI_CLK_13M ,4, 8, EAT_TRUE, EAT_TRUE);
        }else if( 2 == param2)
        {
            eat_spi_read(buffer,1);
            eat_trace("read spi data=%d",buffer[0]);
        }else if( 3 ==  param2)
        {
            eat_spi_write_read(buffer,1, buffer, 1);
            eat_trace("write read spi data=%d",buffer[0]);
        }else if( 4 ==  param2)
        {
            eat_trace("read gpio data=%d",eat_gpio_read(EAT_PIN30_PCM_OUT ));
        }
    }
    return EAT_TRUE;
}
