...
eat_bool simcom_sms_decode_pdu(u8 * data)
{
    ...
    if( eat_decode_sms_pdu(ptr_pdu, pdu_len,&sms_pdu))
    {
        ...
        concatInfo.ref = sms_pdu.tpdu.concat_info.ref;//Ӷűʶ
        concatInfo.seg = sms_pdu.tpdu.concat_info.seg;//Ӷк
        concatInfo.total_seg = sms_pdu.tpdu.concat_info.total_seg;//Ӷܸ
        userDataLen = sms_pdu.tpdu.data.deliver_tpdu.user_data_len;//ǰŵݳ
        sms_orig_address_convert(sms_pdu.tpdu.data.deliver_tpdu.orig_addr, szDestAddr);//ȡ绰
        sms_scts_to_data_time(sms_pdu.tpdu.data.deliver_tpdu.scts,&Date,&Time);//ȡʱ
        ...
        else
        {
            ...
            //ݴs_smsContactData.smsContactDataInfo[0].useData
...
eat_bool eat_module_test_sms(u8 param1, u8 param2)
{
    ...
    {
        if(param2 == 1)//AT+CMGR=1
		{
            simcom_sms_msg_read(1, cmd_cmgr_cb);
		}
    }
    ...
    else if(param1 == 7)//Contact sms receive ,at+cnmi=2,2
    {
        if(param2 == 1)
		{
            u8 ptrPDU[]= "0051000D91683185959606F50008010A0500032302014F60597D";
            u16 len = 25;
            simcom_sms_pdu_send(ptrPDU, len,cmd_cmgs_cb);
		}
        else if(param2 == 2)
